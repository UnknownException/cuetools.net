...
public sealed partial class CoverViewer : UserControl
{
    public CoverViewer()
    {
        InitializeComponent();
    ...
    {
        if (sender is Image { DataContext: CoverViewAlbumViewModel cover }
            && DataContext is CoverViewerViewModel viewModel)
        {
            viewModel.SelectCoverCommand.Execute(cover);
        }
    }
}